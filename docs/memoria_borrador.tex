...
% [PENDIENTE: completar con los resultados finales una vez analizado el experimento completo]

En este TFG hemos diseñado, implementado y ejecutado un benchmark para analizar sesgos relacionados con la discapacidad en modelos de IA generativa en español. El enfoque contrafactual nos ha permitido comparar de forma sistemática las respuestas de cuatro modelos ante las mismas consultas con y sin referencia explícita a la discapacidad.

Ya durante el desarrollo del pipeline pudimos observar ejemplos muy claros de los distintos tipos de sesgo que pueden aparecer. En las pruebas preliminares con el prompt de rechazo laboral, los cuatro modelos mostraron comportamientos distintos entre sí: RigoChat cambiaba sutilmente el tono volviéndose más frío y formal, Llama añadía una nota de inclusión no solicitada, Gemini era consciente de que podía sesgar la respuesta e intentaba evitarlo activamente, y Salamandra llegaba a justificar el rechazo argumentando que la silla de ruedas dificultaba el desempeño del puesto (un puesto de Analista Financiero, donde la discapacidad física no tiene ninguna relevancia). Estos resultados son solo un ejemplo, pero muestran que el problema es real y que se manifiesta de formas muy distintas según el modelo.

% [PENDIENTE: completar con el análisis completo del experimento]
...
\begin{itemize}
    ...
    \item \textbf{Técnicas de mitigación}: una vez identificados los patrones de sesgo, explorar cómo reducirlos mediante ajuste del sistema de instrucciones u otras técnicas de post-procesado.
